...
1. ¿Qué herramienta se utiliza para definir y ejecutar aplicaciones Docker de múltiples contenedores mediante un archivo YAML?
...
2. Sobre la utilidad ab (Apache Benchmark)
...
3. ¿Qué información se espera encontrar en el directorio `/var/log` de un sistema Linux?
...
5. Para editar el archivo `/etc/sudoers` de forma segura, ¿qué comando se recomienda utilizar?
...
6. ¿Qué comando se utiliza para crear un Physical Volume (PV) en un sistema LVM?
...
7. En el "Gaussian Random Timer" configurado con una "Deviation" de 100.0 ms y un "Constant Delay Offset" de 300 ms, ¿cuál es la implicación principal de usar este temporizador en comparación con un "Uniform Random Timer" con un rango equivalente?
...
8. ¿Para qué ha utilizado en la práctica el elemento Access Log Sampler de Jmeter?
...
9. ¿En qué path del sistema de fichero linux tenemos acceso a modificar los parámetros del kernel?
...
10. ¿Qué permite el "Modo Bridge" (Adaptador Puente) en la configuración de red de VirtualBox?
...
11. ¿Cuál es una práctica de seguridad recomendada para el acceso con Ansible a los nodos controlados?
...
13. El comando `systemctl status firewalld` en Rocky Linux se utiliza para:
...
14. ¿En qué se basa Ansible para la ejecución remota de comandos?
...
15. ¿Con qué comando arreglamos finalmente la situación que detenía el arranque?
...
16. ¿Qué utilidad tiene el comando `logger` en el contexto de un script de `cron`?
...
17. ¿Cuál es la diferencia fundamental en cómo Docker utiliza los `namespaces` del kernel de Linux en comparación con los `cgroups` para el aislamiento de contenedores?
...
19. El comando `journalctl -f` en Linux se utiliza para:
...
20. Al usar `semanage port -a -t ssh_port_t -p tcp 2222` para un puerto SSH no estándar con SELinux en `enforcing`, ¿qué sucede si este comando no se ejecuta tras cambiar el puerto en `sshd_config` y reiniciar `sshd` (asumiendo que `firewalld` permite el puerto)?
...
21. ¿Cuál es el comando en Ansible para ejecutar tareas únicas y rápidas sin necesidad de un playbook?
...
22. ¿Cuál es el comando de Docker para ejecutar una imagen llamada "hello-world"?
...
23. ¿Qué comando usamos para lanzar un benchmark con Phoronix (por ejemplo, "sudokut")?
...
24. ¿Qué herramienta se utiliza comúnmente junto con Prometheus para la visualización de métricas y la creación de dashboards?
...
25. ¿Cuál de estos NO es un nivel de Cloud Computing mencionado en las transparencias?
...
26. Considerando el "Regular Expression Extractor" para "Obtener JWT token" que usa la expresión `.+` y "Match No." en 0 (Random), si el cuerpo de la respuesta contiene múltiples líneas y el token JWT aparece en una línea específica, ¿qué comportamiento es más probable para la variable "token"?
...
27. Dado este fragmento del archivo de configuración del agente, indique la linea que deberia continuar: ### Option: Server # List of comma delimited IP addresses, optionally in CIDR notation, or DNS names of Zabbix servers and Zabbix proxies. # Incoming connections will be accepted only from the hosts listed here. # If IPv6 support is enabled then '127.0.0.1', '::127.0.0.1', '::ffff:127.0.0.1' are treated equally # and '::/0' will allow any IPv4 or IPv6 address. # 0.0.0.0/0 can be used to allow any IPv4 address. # Example: Server=127.0.0.1,192.168.1.0/24,::1,2001:db8::/32,zabbix.example.com # # Mandatory: yes, if StartAgents is not explicitly set to 0. # Default: Server=
...
28. ¿Cuál es el propósito de la opción `--runtime-to-permanent` en `firewall-cmd`?
...
29. La configuración de una alerta en Grafana incluye la condición principal y el parámetro "for" (ej. for 5m). ¿Cuál es el propósito fundamental de este parámetro "for" para mejorar la calidad de las alertas?
...
30. En un Dockerfile: `COPY ./myapp /usr/src/app`, `WORKDIR /usr/src/app`, `RUN ["npm", "install"]`. Si `myapp/node_modules` se copia, ¿qué comportamiento tendrá `npm install`?
...
31. ¿Qué significa el parámetro -c en ab?
...
32. Si clonas una Máquina Virtual en VirtualBox, ¿qué obtienes?
...
33. Para monitorizar la memoria de una aplicación NodeJS, se usan las métricas nodejs_heap_size_total_bytes y nodejs_heap_size_used_bytes. ¿Qué tipo de visualización en Grafana sería más efectiva para mostrar la relación entre estas dos métricas y el porcentaje de uso del heap?
...
34. ¿Cuál de estas herramientas NO se menciona en el temario del Bloque 2 como una solución de monitorización o pruebas de carga?
...
35. ¿Qué archivo de auth.log contiene entradas más antiguas?
...
36. En el contexto de la configuración de red de una máquina virtual, ¿cuál es uno de los propósitos principales de NAT (Network Address Translation)?
...
37. ¿Qué directorio en Linux es el punto de montaje temporal estándar para sistemas de archivos?
...
38. De cara a realizar un test con muchas hebras y con carga de CPU...
...
39. Para mover el contenido del directorio `/var` a un nuevo volumen LVM montado en `/mnt/nuevo_var` preservando atributos, ¿qué opción es adecuada para el comando `cp`?
...
40. ¿Qué comando se utiliza para editar el archivo crontab del usuario actual?
...
41. Si se ejecuta el comando journalctl -b -p warning y, entre los resultados, se observa un mensaje con nivel de prioridad emerg, ¿qué implicación directa se deriva de la funcionalidad de journalctl según la información proporcionada?
...
42. ¿Qué tipo de métrica de Prometheus es adecuada para representar valores que pueden subir y bajar, como la temperatura actual o la memoria en uso?
...
43. ¿Cuál es el propósito principal de la tarea de Ansible que utiliza el módulo user con los parámetros name: "{{ admin_user }}", groups: wheel, append: yes, shell: /bin/bash, state: present?
...
46. ¿Qué es un "Exporter" en el contexto de Prometheus?
...
47. En el contexto de SSH, ¿cuál es la principal diferencia entre criptografía simétrica y asimétrica?
...
48. ¿Qué se entiende por "idempotencia" en el contexto de Ansible?
...
50. En un entorno de "Cloud Híbrida", ¿qué se combina?
...
51. En una estructura de directorios de Ansible como la mostrada (Primera_parte/ con subdirectorios group_vars/, tasks/, claves/), ¿cuál es el rol típico de group_vars/all.yml?
...
52. ¿Cómo se suelen comunicar los sistemas basdados en microservicios?
...
53. En la configuración de docker-compose.yml proporcionada para desplegar Prometheus y Grafana, el servicio de Grafana expone un puerto al host y depende de Prometheus. ¿Qué par de puertos (HOST:CONTAINER) se utiliza para Grafana y cuál es la implicación principal de la directiva depends_on: - prometheus?
...
54. En el playbook de Ansible, la línea vars_files: - "../vars/{{ group_names[0] }}.yml" intenta cargar variables. ¿Cuál podría ser una consideración importante o posible limitación al usar {{ group_names[0] }} para cargar archivos de variables destinados a ser específicos del host o de un grupo particular?
...
55. Si en JMeter se utiliza un "HTTP Authorization Manager" para configurar la autenticación básica, ¿qué información se suele proporcionar?
...
56. El comando `umount /mnt/midisco` se utiliza para:
...
57. ¿Cómo se comunica el cliente con la aplicación?
...
58. ¿Qué es el "load average" que muestra el comando `top`?
...
59. ¿Qué información específica NO suele estar directamente en el archivo `/proc/cpuinfo`?
...
60. Con `ssh-copy-id`, si el home del usuario remoto o `.ssh` tienen permisos excesivos (ej. `777`), ¿por qué podría el servidor SSH rechazar la autenticación por clave aunque `authorized_keys` sea correcta?
...
61. En la tarea de Ansible: - name: Crear usuario {{ item.nombre }} user: name: "{{ item.nombre }}" groups: wheel ...loop: "{{ usuarios_extra }}"¿Cómo funciona la directiva loop junto con item?
...
62. Si necesitas detener un arreglo RAID activo, por ejemplo /dev/md0, ¿qué comando mdadm usarías?
...
63. Si el colector systemd de node_exporter no se habilita explícitamente (es decir, no se añade --collector.systemd a sus argumentos de inicio), ¿cuál sería el resultado más probable al intentar ejecutar la consulta PromQL node_systemd_unit_state{name="sshd.service"} en Prometheus?
...
64. ¿Qué indica el valor `wa` (I/O wait) en la línea de uso de CPU del comando `top`?
...
65. ¿Qué ventaja ofrece Podman sobre Docker según se menciona en el temario?
...
66. ¿Qué característica principal diferencia a los contenedores de las máquinas virtuales tradicionales?
...
67. ¿Con qué opción de phoronix puedes comprobar el tamaño de un benchmark antes de descargarlo?
...
68. Para hacer permanente una regla añadida con `firewall-cmd` (por ejemplo, abrir un servicio), ¿qué opción se debe utilizar junto con `--add-service`?
...
69. La documentación menciona que el exporter de Prometheus para la API Web se generó empleando prom-client y express-prom-bundle para NodeJS. ¿Cuál es el rol general de estas librerías en una aplicación NodeJS que necesita exponer métricas a Prometheus?
...
70. ¿Qué tipo de RAID utiliza "striping" pero no ofrece redundancia, resultando en pérdida total de datos si un disco falla?
...
71. En JMeter, ¿qué es un "Thread Group" (Grupo de Hilos)?
...
72. Sabiendo que journalctl -p warning muestra mensajes de prioridad warning, err, crit, alert, emerg, ¿cómo se consultarian los logs del arranque actual para ver mensajes que sean err o crit, pero excluyendo explícitamente los de nivel warning y también los más graves como alert o emerg?
...
73. El comando ./jmeter -n -t /ruta/archivo.jmx -l /ruta/archivo_resultados.jtl ejecuta JMeter en modo no-GUI. ¿Cuál es una ventaja CRUCIAL de este modo de ejecución para pruebas de carga de alto volumen en comparación con el modo GUI?
...
74. ¿Para que se usa el comando grep?
...
75. ¿Qué significa cuando vemos en el prompt initramfs?
...
76. ¿Qué compañía es la actual propietaria del SO Solaris?
...
77. Phoronix Test Suite (PTS) es una plataforma para:
...
78. Para la entrega de los dashboards de Grafana, se indica usar la opción "Share Dashboard or Panel" y exportar con "Export for sharing externally". ¿En qué formato se exporta típicamente la definición del dashboard mediante esta opción y cuál es su principal ventaja?
...
79. ¿Qué tipo de partición NO permite tamaño de archivos mayor de 4GB?
...
80. En JMeter, un "Regular Expression Extractor" captura un token de un encabezado de respuesta que puede aparecer varias veces. Con "Match No." en 0 (random), ¿cuál es el desafío para obtener consistentemente el token de la *última* aparición?
...
81. ¿Cuál es el propósito principal de un "Panel" en un dashboard de Grafana?
...
83. ¿Para qué se utiliza la opción `-p` en el comando `ssh`? (Ej: `ssh -p 2222 usuario@servidor`)
...
84. ¿Por qué no hemos usado Im-sensors en las prácticas?
...
85. Considerando la secuencia de comandos LVM: 1. sudo pvcreate /dev/md0, 2. sudo vgcreate vg_datos /dev/md0, 3. sudo lvcreate -L 900M -n lv_datos vg_datos. ¿Cuál es el propósito del segundo comando (sudo vgcreate vg_datos /dev/md0)?
...
86. ¿Qué debemos tener en cuenta antes de ejecutar un benchmark?
...
87. ¿Qué debemos tener instalado en los hosts para usar ansible?
...
88. Considerando el entorno completo (nodo de control y nodos gestionados), ¿qué se necesita fundamentalmente para ejecutar playbooks de Ansible y que estos operen sobre los nodos gestionados?
...
89. En el archivo docker-compose.yml, el servicio grafana tiene la directiva depends_on: - prometheus. Además de influir en el orden de inicio de los contenedores, ¿qué otra facilidad importante proporciona esta directiva en el contexto de una red Docker Compose por defecto?
...
90. Al instalar node_exporter, se descarga un archivo .tar.gz. Si un usuario intenta descomprimir este archivo directamente en /usr/local/src usando tar -xvzf node_exporter.tar.gz -C /usr/local/src sin sudo, ¿cuál es el resultado más probable si el usuario no tiene permisos de escritura en /usr/local/src?
...
92. Después de iniciar máquinas virtuales, un script ejecuta ansible all -i inventory/hosts.ini -m ping. ¿Cuál es el objetivo principal de este comando en ese contexto?
...
93. En el archivo `prometheus.yml`, ¿qué especifica la directiva `scrape_configs`?
...
94. Al ejecutar `docker compose down`, ¿qué sucede con los contenedores definidos en el archivo `docker-compose.yml`?
...
95. ¿Cuál es el comando mdadm correcto para crear un arreglo RAID 1 (mirroring) llamado /dev/md0 con dos dispositivos, como /dev/sdX y /dev/sdY?
...
96. En el contexto del ejercicio de monitorización del servidor Linux, se menciona el uso de la herramienta stress para "poner de manifiesto el funcionamiento de la alarma" asociada al uso de CPU. ¿Cuál es el propósito general de utilizar stress en este escenario?
...
97. Para ejecutar una prueba de JMeter sin la interfaz gráfica, desde la línea de comandos, ¿qué opción se suele usar?
...
98. ¿Cuál es el comando correcto en un sistema basado en DNF (como Rocky Linux) para instalar tanto el servidor como el cliente SSH?
...
99. ¿Cuál es el objetivo de utilizar `snapshots` en VirtualBox?
...
100. Si un `docker-compose.yml` define dos servicios, `webapp` y `database`, y `webapp` tiene `links: - database`, esto permite que:
...
101. Si Ansible configura la línea PermitRootLogin prohibit-password en /etc/ssh/sshd_config, ¿qué implicación tiene para el acceso SSH del usuario root?
...
103. ¿Cuál es el propósito del comando ssh-keygen -t rsa -b 4096?
...
104. Al intentar depurar un problema ocurrido durante el penúltimo arranque (no el actual, sino el anterior a ese), un técnico necesita ver únicamente los mensajes con nivel de prioridad crit o superior. ¿Cuál de los siguientes comandos journalctl es el más preciso y eficiente para esta tarea específica?
...
105. El sistema de ficheros `/proc` en Linux es:
...
106. ¿Cuál es la función principal de `iptables` en Linux?
...
107. El comando `firewall-cmd --reload` se utiliza para:
...
108. ¿Qué comando de Docker se utiliza para detener un contenedor en ejecución sin eliminarlo?
...
109. Al hacer el test de JMeter ¿Dónde generamos carga?
...
110. ¿Qué comando se utiliza para consultar los logs del sistema gestionados por `systemd-journald`?
...
111. Para la monitorización de una API WEB, se requiere crear un panel que muestre la relación entre la memoria heap total de NodeJS y la memoria heap usada. ¿Cuáles son las dos métricas específicas de prom-client mencionadas en la documentación que se utilizarían para construir este panel?
...
112. ¿Cuál es el resultado de ejecutar "ab n 10 c 5 192.168.56.110/"?
...
113. En Munin ¿es posible mostrar gráficas en función del tiempo?
...
114. Si `PermitRootLogin prohibit-password` está en `sshd_config`, y `/root/.ssh/authorized_keys` es correcto, ¿por qué podría fallar un login root con clave?
...
115. Al configurar Grafana con Prometheus, ¿qué se define como "Datasource"?
...
116. En un sistema Linux que utiliza LVM sobre un RAID 1 por software (mdadm) para el directorio `/var`, si uno de los discos físicos del RAID 1 comienza a experimentar fallos de escritura intermitentes que no son inmediatamente detectados como un fallo total por `mdadm`, ¿cuál es el riesgo más significativo?
...
118. ¿Qué representa `scrape_interval: 5s` en la configuración global de `prometheus.yml`?
...
119. Según el temario, ¿cuál es una de las ventajas de la arquitectura de microservicios relacionada con la tecnología?
...
120. En JMeter, "CSV Data Set Config" con "Recycle on EOF:True", "Stop thread on EOF:False", CSV de 100 líneas, 5 hilos, Loop 30. ¿Qué impacto tiene "Sharing mode: Current thread group" vs "All threads" en la unicidad de datos por hilo?
...
121. ¿A qué opción nos debemos ir si queremos empezar a monitorizar una nueva maquina desde el panel de Zabbix?
...
122. En la configuración de red de una máquina virtual en VirtualBox, ¿qué modo de red permite la comunicación con el equipo Anfitrión y otras posibles máquinas virtuales, pero no necesariamente con Internet directamente?
...
123. Para monitorizar el estado de servicios como SSHD y Apache Httpd (Activo/Inactivo) en Grafana usando Node Exporter, ¿qué métrica de Node Exporter (con el colector systemd habilitado) podría ser útil?
...
124. ¿Qué contienen los CSV introducidos en JMeter?
...
125. ¿Cuál de los siguientes no es un monitor?
...
126. En el contexto del `docker-compose.yml` para la aplicación de JMeter, la directiva `build: ./nodejs` para el servicio `nodejs` significa:
...
127. ¿Cuál es la ruta principal del archivo de configuración de logrotate en la mayoría de los sistemas Linux?
...
128. ¿Para qué se utiliza el comando `ping -c 4 google.com`?
...
129. En la petición HTTP "Login Administradores", se envían los parámetros `login` y `password` con "Content-Type" `text/plain`. Si el servidor espera estos parámetros en el cuerpo de una solicitud POST pero con un "Content-Type" de `application/x-www-form-urlencoded` o `application/json`, ¿qué problema podría surgir?
...
130. El comando `firewall-cmd --list-services` muestra:
...
131. ¿Qué se recomienda en las prácticas de ISE para asegurar el prompt de la shell en las capturas de pantalla?
...
132. ¿Qué puerto es el usado por defecto por los servidores web y debe ser abierto en el firewall para permitir su acceso público?
...
133. ¿Cuál es el tamaño por defecto a partir del cual IIS empieza a comprimir las páginas?
...
134. La configuración de Grafana en docker-compose.yml incluye ports: - "4000:3000". ¿Cuál es el propósito y efecto de este mapeo de puertos específico?
...
135. ¿Qué herramienta de monitorización clásica en Linux presenta una interfaz de texto para ver procesos y uso de recursos en tiempo real?
...
136. Si un Volume Group (VG) en LVM se queda sin espacio, ¿qué se podría hacer para ampliarlo si se dispone de un nuevo disco o partición?
...
137. Para que Prometheus pueda recolectar métricas de un servicio (ej. Node Exporter), el servicio debe:
...
138. ¿Qué significa "[2/1][U_]" dentro del archivo de estado del md?
...
139. Al interpretar los resultados de Apache Benchmark (ab) al comparar dos servidores HTTP, si el "Time per request (mean, across all concurrent requests)" es significativamente menor para el Servidor A que para el Servidor B con alta concurrencia, ¿qué indicaría esto primordialmente?
...
140. Al importar un dashboard en Grafana usando un ID, ¿de dónde se obtiene típicamente ese dashboard?
...
141. ¿Qué es lo primero que debemos tener en mente cuando vamos a seleccionar un benchmark?
...
142. ¿Cuál de los siguientes es un servidor web?
...
143. En LVM, ¿qué comando se utiliza para crear un Volume Group (VG)?
...
145. ¿Cuál de los siguientes comandos de Linux te permite averiguar la IP del equipo?
...
146. ¿Qué comando de LVM se utiliza para mostrar información sobre los Logical Volumes (LVs) existentes?
...
147. ¿Para qué es la primera petición que se realiza en la aplicación de prácticas?
...
148. ¿Qué hay que pulsar para matar un proceso desde top?
...
149. ¿En qué puerto escucha el agente de Zabbix por defecto?
...
150. Según el desarrollo del ejercicio de Ansible, ¿cuándo es más apropiado usar comandos Ad-Hoc de Ansible en lugar de playbooks?
...
151. En el contexto de Grafana, ¿qué es un "dashboard"?
...
152. ¿Qué error es posible cometer al ejecutar ab por un despiste?
...
153. ¿Cuáles son soluciones de virtualización?
...
154. ¿Qué nivel de RAID es conocido como "Mirroring" y duplica los datos en dos o más discos?
...
155. La métrica http_request_duration_seconds_bucket se menciona para la monitorización de tiempos de respuesta de una API WEB. ¿Qué tipo de información fundamental proporciona esta métrica de tipo histograma para el análisis del rendimiento?
...
156. El comando `docker exec -it [nombre_contenedor] sh` permite:
...
157. La tarea de Ansible "Asegurar que el servicio {{ web_service }} está iniciado y habilitado" utiliza el módulo service con state: started y enabled: true. ¿Qué garantiza esta configuración?
...
158. ¿Cuál es una de las principales ventajas de `htop` sobre `top`?
...
159. ¿Por qué se podría cambiar el puerto predeterminado de SSH (22) a otro valor, como Port 2222, en la configuración del servidor SSHD?
...
160. ¿Con que comando iniciamos la aplicación antes de hacer el test de Jmeter?
...
161. En el contexto del temario, ¿para qué se configura el `prompt` de la shell de una manera específica en las máquinas virtuales de prácticas?
...
162. ¿Qué representa la opción `-n` en el comando `ab -n 1000 -c 100 http://example.com/`?
...
163. ¿En qué directorio se sitúan por defecto los ficheros de registro de actividad de Linux?
...
164. ¿Qué función principal realiza el comando ssh-copy-id usuario@servidor?
...
165. ¿Cuál de las siguientes NO es una responsabilidad de LVM?
...
166. En Ansible, un playbook tiene un handler notificado por una tarea que ejecuta un script con `ansible.builtin.command`. Si este script devuelve un código de salida erróneo, pero la tarea tiene `ignore_errors: true`, ¿se ejecutará el handler?
...
167. ¿Cómo se autentica un usuario en la API para pedir información a la BD?
...
168. En `prometheus.yml`, el `scrape_interval` global es 15s. Un job para `node_exporter` no define su propio `scrape_interval`. Un panel en Grafana usa `rate(node_cpu_seconds_total[1m])`. ¿Cómo afecta esto la precisión?
...
169. ¿Qué significa el parámetro -c en ab?
...
171. ¿Cuál es el objetivo de "asegurar" el servicio SSH, según lo mencionado en el temario?
...
173. El comando sudo dnf install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin instala Docker Engine y varios componentes. ¿Cuál es la implicación principal de incluir paquetes como docker-buildx-plugin y docker-compose-plugin en esta instalación?
...
174. En JMeter, elementos como "Login Alumnos", "Obtener JWT Token" y "Recuperar datos alumnos" están organizados bajo un elemento padre llamado "Alumnos". Este elemento "Alumnos" es probablemente un:
...
175. ¿Cuál es el propósito de la siguiente línea en el archivo /etc/fstab: /dev/mapper/raid1-rvar /var ext4 defaults 0 0?
...
176. ¿Qué distribución es derivada de Red Hat?
...
177. La persistencia de datos en Grafana (dashboards, configuraciones) cuando se ejecuta en Docker se logra típicamente mediante:
...
178. ¿Cuál de las siguientes es una característica de la pila ELK (Elasticsearch, Logstash, Kibana)?
...
180. ¿Cual de las siguientes afirmaciones es cierta sobre la Fundación Apache?
...
181. En las variables de Ansible ssh_pub_key_admin: "{{ lookup('file', 'claves/id_rsa_admin.pub') }}", ¿qué función realiza lookup('file', ...)?
...
182. En el archivo `prometheus.yml`, la sección `static_configs` dentro de un `job_name` se utiliza para:
...
183. Al cambiar el puerto por defecto de SSHD, además de modificar `/etc/ssh/sshd_config`, ¿qué otra acción es crucial?
...
184. ¿Para que sirve el fichero create.sql.gz de Zabbix?
...
185. En un archivo crontab, la expresión `*/5* ** * /ruta/a/mi/script.sh` significa:
...
188. ¿Cuál de las siguientes es una característica de la criptografía de clave simétrica?
...
189. ¿Para qué se utiliza el comando sudo systemctl isolate rescue.target en un sistema Linux que utiliza systemd?
...
190. ¿Cuál es la diferencia entre un "RAID por Software" y un "RAID por Hardware"?
...
191. ¿Qué ventaja principal ofrecen los contenedores sobre las máquinas virtuales en términos de uso de recursos?
...
192. ¿Qué es un "playbook" en Ansible?
...
193. El protocolo `sftp` (Secure File Transfer Protocol) funciona sobre:
...
194. ¿Cómo se denominan los archivos de scripts de ansible?
...
196. ¿Con qué programa accedemos a la información de los monitores hw en Linux?
...
197. Para crear un arreglo RAID 5 llamado /dev/md0 utilizando tres discos (/dev/sdX, /dev/sdY, /dev/sdZ), ¿cuál sería el comando mdadm apropiado?
...
198. ¿Cuál es la principal desventaja de RAID 0 (Striping)?
...
199. El comando `usermod` en Linux se utiliza principalmente para:
...
200. ¿Qué tecnología permite agrupar varios dispositivos de almacenamiento creando un nuevo dispositivo virtual con capacidades extendidas?
...
201. ¿Qué significa que un sistema de ficheros como ext4 o XFS sea "transaccional"?
...
202. En el contexto de Docker, ¿qué es una "imagen"?
...
203. En JMeter, ¿para qué se utiliza un "CSV Data Set Config"?
...
204. Un script programado en el crontab del usuario para ejecutarse cada minuto (** ** * /home/user/script.sh) tiene como objetivo registrar la carga del sistema usando logger -t ISE "Carga: $(uptime)". El script tiene permisos de ejecución. Si no aparecen logs con la etiqueta "ISE" en journalctl -t ISE, pero otros logs del sistema sí se registran correctamente, ¿cuál de las siguientes es una causa raíz probable y específica del problema con este cron job?
...
205. ¿Cuál de los siguientes ficheros dentro del sistema de archivos /proc sería el más directo y apropiado para analizar la distribución y el uso actual de la memoria RAM y swap del sistema, incluyendo detalles como memoria total, libre, buffers y caché?
...
206. ¿Qué indica el estado "zombie" de un proceso en la salida de `top`?
...
208. En el `docker-compose.yml` para Grafana y Prometheus, `depends_on: - prometheus` en la definición del servicio Grafana significa:
...
209. ¿Qué comando emplearía en CentOS para comprobar si hay procesos del servidor web ejecutándose?
...
210. ¿Cuál es un riesgo de configurar `PermitRootLogin yes` en `sshd_config` sin otras medidas de seguridad?
...
211. ¿Tiene sentido aplicar el test a la misma máquina donde ejecutamos ab?
...
212. ¿Cuál es el puerto por defecto en el que Prometheus expone su propia interfaz web y métricas?
...
213. ¿Cuál de las siguientes configuraciones de la variable PS1 en bash mostraría el nombre de usuario, el hostname, la hora actual (HH:MM:SS) y el directorio de trabajo actual, seguido de $ o #?
...
214. ¿Cuál es una de las principales razones para usar RAID por hardware en lugar de RAID por software?
...
215. ¿Qué diferencia hay entre la opción run y la opción benchmark cuando usamos phoronix?
...
216. Al instalar Apache (httpd) en Rocky Linux, ¿qué comando se usa para iniciar el servicio?
...
217. ¿Cuáles son las ventajas de un contenedor frente a una Máquina Virtual?
...
218. Al hacer el test de Jmeter ¿Dónde generamos carga?
...
219. ¿Qué representa el "hardware virtual" en un entorno de virtualización?
...
220. El comando VBoxManage startvm srv1 --type headless se utiliza en un script. ¿Cuál es su función?
...
221. En el contexto de la analogía "proceso vs hebra", ¿a qué se asemejan más los contenedores?
...
223. ¿Qué comando usarías para ver los logs del demonio SSHD (sshd) con el fin de diagnosticar problemas, mostrando información extendida?
...
224. En el contexto de Docker, ¿qué significa `EXPOSE 3000` en un Dockerfile?
...
226. ¿Qué tecnología subyacente del kernel de Linux utilizan los contenedores para aislar los sistemas de archivos y los procesos?
...
227. ¿Cuál de los siguientes programas no es un benchmark de sistema?
...
228. ¿Qué significan las siglas JFS del sistema de ficheros empleado en prácticas?
...
229. En LVM, ¿cuál es el componente que representa los dispositivos de almacenamiento físico como discos duros?
...
231. Al migrar el directorio `/var` a un nuevo Logical Volume (LV) formateado con `ext4` sobre un RAID 1, ¿por qué es crucial `cp -a /var/* /mnt/` en lugar de `cp -r` en modo de mantenimiento?
...
232. ¿Qué directorio en Linux contiene los archivos de arranque del sistema?
...
233. En JMeter un "JWT Token" obtenido tras el login se utiliza probablemente en peticiones subsiguientes dentro de un:
...
234. Si ejecutamos ab -n 1000 192.168.56.105 veremos...
...
235. ¿Qué comando emplearía para establecer una conexión cifrada con un equipo remoto?
...
236. ¿Cuál es la principal utilidad del comando `hostnamectl` en Linux?
...
237. ¿Para qué se ha utilizado en la práctica el elemento Gaussian Random Timer de JMeter?
...
238. ¿Cuál es el comando utilizado en Rocky Linux para gestionar el firewall desde la línea de comandos?
...
239. Si queremos utilizar el script pruebaEntorno.sh del repositorio...
...
240. ¿Cuál es la característica principal de RAID 0 (Striping)?
...
241. ¿Para qué se utiliza el comando `nmap` en el contexto de la seguridad de servidores?
...
242. Si se modifica la `PS1` (variable del prompt de Bash) en el archivo `.bashrc`, ¿cuándo surten efecto los cambios?
...
243. ¿Qué comando se utiliza para cambiar al "modo mantenimiento" o runlevel 1 en un sistema Linux con systemd?
...
244. ¿Qué archivo hay que consultar para ver quien se ha autorizado como su en Debian/Ubuntu?
...
245. ¿Con que comando podemos consultar la salida de un timer como el del guión?
...
246. Observando el "Test Plan" donde se definen las variables HOST (172.17.0.1) y PORT (3000), si un "Thread Group" específico dentro de este plan define una "User Defined Variables" con la variable HOST con valor "localhost", ¿qué valor de HOST utilizará un "HTTP Request" sampler dentro de ese "Thread Group" que referencia `${HOST}`?
...
247. La directiva `PermitRootLogin` en el archivo de configuración de SSHD (`sshd_config`) controla:
...
248. ¿Qué tipo de Cloud Computing es "PaaS" (Platform as a Service)?
...
249. Si se inicia node_exporter con los colectores --collector.systemd y --collector.filesystem habilitados, además de sus colectores por defecto, ¿qué tipo de información adicional específica del sistema se podría esperar que exponga para Prometheus?
...
250. ¿Qué comando se utiliza en Rocky Linux para configurar las interfaces de red si se usa NetworkManager desde la CLI?
...
251. La métrica node_cpu_seconds_total de node_exporter es un contador que registra el tiempo de CPU acumulado en diferentes modos (idle, user, system, etc.). En la consulta PromQL para el uso de CPU: (1 - avg by (instance) (rate(node_cpu_seconds_total{job="rocky_linux_server",mode="idle"}[5m]))) * 100, ¿cuál es el propósito principal de la función avg by (instance)?
...
252. El script de cron debe generar un mensaje de log incluyendo la "<Carga actual del Sistema>". ¿Cuál de los siguientes ficheros del sistema de archivos /proc proporcionaría directamente los valores de la media de carga (load average) que comúnmente se usan para este propósito?
...
253. ¿Cuál es el propósito del comando `ssh-keygen`?
...
254. La métrica node_cpu_seconds_total es fundamental para calcular el uso de CPU. Dado que es un contador (counter) que solo aumenta, ¿por qué es esencial aplicar la función rate() (o similar como irate()) a esta métrica en PromQL antes de intentar calcular un porcentaje de uso?
...
255. En la configuración de una regla de alerta en Grafana para el uso de CPU, se establece "Evaluate every 1m" y "for 5m". ¿Cuál es la diferencia fundamental en el comportamiento de la alerta si el parámetro "for 5m" se omitiera o se estableciera en "for 0m"?
...
256. ¿Qué comando instala el programa midnight-commander?
...
257. Si en el "HTTP Authorization Manager" se activa la opción "Clear auth on each iteration?", ¿cuál sería el impacto en un escenario donde un hilo realiza múltiples peticiones HTTP dentro de una misma iteración del bucle del Thread Group, todas ellas requiriendo la misma autorización básica?
...
259. En JMeter, ¿qué elemento se utiliza para simular que un usuario realiza una petición HTTP a un servidor?
...
260. Al configurar una alerta en Grafana basada en `avg_over_time(process_cpu_seconds_total[5m]) > 0.75`, ¿qué impacto tiene la elección del "For" (duración) en la alerta si la métrica fluctúa cerca del umbral?
...
261. La utilidad `vmstat` en Linux proporciona información sobre:
...
262. Si se desea que Prometheus monitoree una instancia de node_exporter que se ejecuta en el host de Docker (o en otro contenedor accesible en la red Docker como 'node-exporter-host:9100'), ¿cómo se modificaría principalmente el archivo prometheus.yml para añadir este nuevo objetivo de scraping?
...
263. ¿Cuál es la función principal de Prometheus en una pila de monitorización?
...
264. Si Prometheus no puede acceder a un `target` para hacer `scraping`, ¿qué estado mostrará típicamente para ese `target` en su interfaz web?
...
265. En un Dockerfile, ¿qué instrucción se utiliza para especificar la imagen base a partir de la cual se construirá la nueva imagen?
...
266. En la gestión de iptables, ¿qué acción realiza el comando iptables -L?
...
267. ¿Cuál es el propósito de `logrotate`?
...
268. De cara a realizar un test con muchas hebras y con carga de CPU...
...
269. ¿Qué comando se utiliza para crear un Logical Volume (LV) en LVM, especificando su tamaño y nombre?
...
270. Si un equipo de desarrollo prioriza ejecutar contenedores sin privilegios de root por defecto y busca minimizar la superficie de ataque eliminando procesos daemon en segundo plano, ¿qué herramienta sería la más adecuada para estos requisitos?
...
271. ¿Dónde se tiene que añadir el token de login?
...
272. ¿Qué tipo de "Sampler" en JMeter se usaría para realizar una petición a una base de datos mediante JDBC?
...
273. Respecto a la escalabilidad en microservicios, ¿cuál de las siguientes afirmaciones distingue más sutilmente la escalabilidad horizontal de la vertical, más allá de la simple definición?
...
274. ¿Cuál de los siguientes programas es un editor de textos?
...
275. ¿Dónde está el archivo de configuración de SSH?
...
276. ¿Qué es un "benchmark" en el contexto de la ingeniería de servidores?
...
277. La consulta PromQL proporcionada para calcular el "Uso Total CPU (%)" en un panel de Grafana es (1 - avg by (instance) (rate(node_cpu_seconds_total{job="rocky_linux_server",mode="idle"}[5m]))) * 100. ¿Qué representa específicamente la parte rate(node_cpu_seconds_total{...mode="idle"}[5m]) en este contexto?
...
278. ¿Qué aporta ssh respecto a Telnet?
...
279. En el contexto de SSH, ¿qué significa "criptografía asimétrica"?
...
280. ¿Es posible ajustar el comportamiento del sistema modificando los parámetros del kernel en tiempo de ejecución?
...
281. ¿Para qué se ha utilizado la petición GET en la práctica?
...
282. Al configurar un RAID por software en Linux, ¿cuál es un paso crucial que generalmente precede al montaje del dispositivo RAID (por ejemplo, /dev/md0) en el sistema de archivos?
...
284. La consulta PromQL para el uso de CPU es (1 - avg by (instance) (rate(node_cpu_seconds_total{job="rocky_linux_server",mode="idle"}[5m]))) * 100. Si se eliminara el filtro mode="idle", resultando en rate(node_cpu_seconds_total{job="rocky_linux_server"}[5m]), ¿qué representaría esta parte modificada de la consulta?
...
285. ¿Qué es el "Filesystem Hierarchy Standard (FHS)" en Linux?
...
286. ¿Cuál es una de las principales ventajas de usar `docker-compose` para una aplicación con múltiples servicios (ej. app web + base de datos)?
...
287. Si el servidor SSH está escuchando en un puerto no estándar (ej. 2222), ¿cómo se especificaría este puerto al usar ssh-copy-id para copiar una clave pública?
...
288. En JMeter, ¿qué elemento se utiliza para manejar las credenciales de autenticación básica HTTP de forma global en el plan de pruebas?
...
289. ¿Qué representa `ansible_host` en un archivo de inventario de Ansible?
...
290. En la tarea de Ansible "Asegurar que Apache (httpd) está detenido...", ¿cuál es el propósito de la cláusula when: web_package == "nginx"?
...
291. El comando `mkfs.ext4 /dev/vg_datos/lv_app` se utiliza para:
...
292. La instalación de Apache Benchmark (ab) en sistemas basados en Debian/Ubuntu se realiza comúnmente con sudo apt install apache2-utils -y. ¿Qué indica el uso de apache2-utils en lugar de un paquete llamado, por ejemplo, ab-standalone?
...
293. ¿Qué editor de textos tiene modo edición y modo comandos?
...
294. ¿Cuál de los siguientes directorios en Linux está destinado a almacenar datos variables como registros (logs) y colas de impresión, que pueden crecer con el tiempo?
...
296. En la tarea "Crear una página de prueba para Apache", se utiliza el módulo copy con el parámetro content y se incluye notify: Reiniciar Apache. ¿Cuál es la función combinada de estos elementos?
...
297. Al usar el módulo firewalld de Ansible con state: enabled y immediate: yes para abrir un puerto, ¿qué significa immediate: yes?
...
298. ¿Qué tipo de métrica de Prometheus es `http_requests_total` típicamente?
...
299. Un Volume Group (VG) de LVM contiene un PV en SSD y otro en HDD. Se crea un LV sin política de asignación y luego se extiende. ¿Cuál es la implicación crítica para el rendimiento?
...
300. ¿En base a qué elemento podremos emitir juicios sobre un el rendimiento de una máquina tras ejecutar el benchmark?
...
301. ¿Cuál es una ventaja de ejecutar Phoronix Test Suite dentro de un contenedor Docker, según el temario?
...
302. Si se cambia el puerto de SSH a 2222 y SELinux está activo, ¿qué comando se utiliza para permitir este nuevo puerto para el servicio SSH en SELinux?
...
303. Al definir una alerta en Grafana, como la "Alerta CPU Alta (ISM)", se incluye un campo "Summary". ¿Cuál es la función principal de este campo en el ciclo de vida de una alerta?
...
304. ¿Qué es cierto para Apache Httpd?
...
305. ¿Cuál es la principal ventaja de RAID 1 (Mirroring) sobre RAID 0 (Striping)?
...
306. ¿Qué efecto tiene ignore_errors: true en una tarea de Ansible, como la utilizada al intentar detener un servicio que podría no estar instalado?
...
307. Si se utiliza el comando iptables -F, ¿qué sucederá con la configuración del firewall?
...
308. ¿Cuál es la principal ventaja de RAID 1 (Mirroring)?
...
309. Apache JMeter es una aplicación diseñada principalmente para:
...
310. La arquitectura de microservicios permite que cada servicio utilice su propio stack tecnológico. Si bien esto ofrece gran flexibilidad, ¿cuál de los siguientes escenarios describe mejor una consecuencia directa y a menudo desafiante de esta característica en un entorno de producción complejo?
...
311. En Docker Compose, si el servicio `webapp` tiene `depends_on: [db]`, ¿qué garantiza Compose sobre el servicio `db` antes de iniciar `webapp`?
...
312. ¿Cuál es la principal diferencia entre una máquina virtual (VM) y un contenedor Docker?
...
313. Docker Compose utiliza Docker como motor de contenedores y requiere un daemon, mientras que Docker tiene soporte rootless limitado y Docker Compose no lo soporta. ¿Qué implicación funcional o de dependencia se deriva de esto para Docker Compose respecto a la ejecución rootless?
...
314. ¿Cuál es el propósito principal de Grafana en una pila de monitorización con Prometheus?
...
315. El servicio SSH (SSHD) por defecto utiliza el puerto:
...
316. ¿Para qué se plantean 2 hebras en la aplicación de JMeter?
...
317. En Docker, ¿qué es un `Dockerfile`?
...
318. ¿Qué es "stress" en el contexto de las pruebas de rendimiento en Linux?
...
319. Al definir un servicio en `docker-compose.yml` con `image: mongo:6`, Docker Compose:
...
320. ¿Cuál es el comando para ver el contenido del archivo de configuración del demonio SSH en un sistema típico de Linux?
...
321. ¿Qué herramienta se menciona en el temario para ejecutar benchmarks que está disponible como imagen Docker (phoronix/pts)?
...
322. Sobre la compresión del contenido Http:
...
323. ¿Qué comando se utiliza para listar todas las imágenes Docker descargadas localmente?
...
324. Un benchmark, como NameBench para DNS, está diseñado para medir el rendimiento. ¿Cuál es el valor fundamental de ejecutar un benchmark específico para un componente (DNS) en lugar de un benchmark general del sistema?
...
325. Al monitorizar un servidor Linux con Node Exporter y Prometheus, ¿qué tipo de información se puede visualizar en Grafana?
...
327. El archivo `~/.ssh/known_hosts` en un cliente SSH se utiliza para:
...
329. En JMeter, si deseas extraer un valor de una respuesta JSON (por ejemplo, un token JWT), ¿qué tipo de Post-Procesador podrías usar?
...
331. En Ansible, `my_package: httpd` está en `group_vars/all.yml` y `my_package: nginx` en `host_vars/server1.yml`. La tarea `ansible.builtin.package: name={{ my_package }}` ¿qué instalará en `server1`?
...
332. ¿Qué es Docker en el contexto de los contenedores, según se menciona en las transparencias?
...
333. ¿Para qué sirve la opción `-a` del comando `cp` (ej: `cp -a /origen /destino`)?
...
334. ¿Cuál es el propósito del archivo `/etc/fstab` en Linux?
...
335. ¿Cuál es la diferencia entre top y htop?
...
336. El comando `useradd` en Linux se utiliza para:
...
337. ¿Qué información muestra la columna `%CPU` en la salida del comando `top`?
...
339. Al configurar una alarma en Grafana para el uso de CPU, si la condición es "cuando la media del uso de CPU supere el 75% durante 5 minutos", ¿qué implica el "durante 5 minutos"?
...
340. Al configurar el servicio SSHD, ¿cuál es el efecto de establecer la directiva PermitRootLogin no en el archivo sshd_config?
...
342. ¿Qué es FaaS (Function as a Service) en el contexto de Cloud Computing?
...
343. ¿Cuál es una ventaja de usar LVM (Logical Volume Manager)?
...
344. Si un servicio como node_exporter falla al iniciar debido a una denegación de SELinux, y cambiar el contexto del binario a bin_t no resuelve el problema, ¿cuál de las siguientes acciones sería un paso lógico de diagnóstico adicional para entender la causa raíz según las prácticas comunes de SELinux?
...
345. El "Node Exporter" de Prometheus se utiliza para:
...
346. ¿Cuál es el comando mdadm para crear un arreglo RAID 0 (striping) con el nombre /dev/md0 utilizando dos discos, por ejemplo, /dev/sdX y /dev/sdY?
...
347. El sistema de archivos /proc en Linux es una fuente crítica de información dinámica del sistema. Si un script necesita obtener el tiempo exacto que el sistema lleva encendido desde el último arranque, ¿qué característica de /proc/uptime es más relevante para esta tarea?
...
349. En un sistema Linux que interpreta estrictamente la sintaxis cron tradicional de 5 campos (minuto, hora, día del mes, mes, día de la semana), ¿cuál sería el comportamiento de una entrada crontab como 1 ** ** * /ruta/script.sh que, según la documentación inicial, pretendía una ejecución específica?
...
350. Si se desea que un servicio como `httpd` o `sshd` se inicie automáticamente al arrancar el sistema en Rocky Linux, ¿qué comando de `systemctl` se utiliza?
...
351. En la jerarquía estándar del sistema de archivos de Linux, ¿cuál es la función principal del directorio /etc?
...
352. ¿Cuál de los siguientes NO es un sistema de archivos comúnmente soportado y utilizado en Linux?
...
353. Para conceder privilegios de administración a un usuario en Rocky Linux, permitiéndole usar `sudo`, ¿a qué grupo se suele añadir?
...
354. ¿Dónde hemos especificado los nombres de los servidores o hosts en ansible?
...
355. ¿Cuál es el propósito de un "Listener" como "View Results Tree" en JMeter?
...
356. Para la práctica de configuración de red en Rocky Linux, se pide que la interfaz Host-Only tenga una IP:
...
357. ¿Cuál es la función de la directiva `CMD ["npm", "start"]` en un Dockerfile para una aplicación Node.js?
...
358. En un `docker-compose.yml`, ¿qué especifica la sección `ports: - "8080:80"` para un servicio?
...
359. En la consulta PromQL rate(node_cpu_seconds_total{...}[5m]), el selector de rango [5m] especifica el intervalo de tiempo sobre el cual se calcula la tasa. ¿Cómo influye este intervalo en los puntos de datos utilizados por la función rate?
...
360. Un administrador necesita revisar los logs de todos los arranques registrados, excepto el actual, para buscar mensajes con prioridad alert. Antes de ejecutar cualquier comando journalctl, verifica el estado de systemd-journald y lo encuentra "active (running)". ¿Cuál sería la aproximación más directa y adecuada con journalctl para esta tarea, considerando que podrían existir múltiples arranques anteriores?
...
361. La "Red NAT" en VirtualBox es útil principalmente para:
...
363. En un archivo de inventario de Ansible como hosts.ini, ¿qué significa la sección [servidores:vars] seguida de ansible_user=ismMV01?
...
364. ¿Cuál es el propósito de la directiva `volumes` en un archivo `docker-compose.yml` al definir un servicio como Prometheus o Grafana, según el ejemplo del temario?
...
365. Un script de Ansible usa `ansible.builtin.lineinfile` con `state: present` para asegurar una línea en `/etc/security/limits.conf`. Si se ejecuta repetidamente y la línea ya existe y coincide, ¿cuál es el comportamiento y cómo afecta al estado "changed"?
...
366. El archivo prometheus.yml tiene una configuración global: scrape_interval: 5s. Si se quisiera que un job específico para node_exporter tuviera un intervalo de recolección de 15 segundos en lugar de 5, ¿cómo se configuraría esto para ese job en particular?
...
367. Al añadir Prometheus como fuente de datos en Grafana (ambos en contenedores Docker), la configuración de la URL HTTP es http://prometheus:9090 y el modo de Acceso (Access) suele ser "Server". ¿Por qué es preferible el modo "Server" en este escenario en lugar de "Browser"?
...
368. En Ansible, el archivo de "inventario" se utiliza para:
...
369. ¿Cómo se instala un text en phoronix?
...
371. ¿Qué tipo de información suele contener el directorio `/var` en un sistema Linux?
...
372. ¿Cuál de las siguientes afirmaciones es cierta?
...
373. ¿Cuál es uno de los objetivos de monitorizar los "Tiempos de respuesta de los endpoints de la API" (ej: `http_request_duration_seconds_bucket`)?
...
374. ¿Dónde se especifican las credenciales de cada usuario para la petición de autenticación?
...
375. ¿Qué es el "scraping" en el contexto de Prometheus?
...
376. Al monitorizar un sistema con `top`, si se observa un valor consistentemente alto en `%wa` (I/O wait) y valores bajos en `%us` (user) y `%sy` (system), pero la memoria RAM y swap no están saturadas, ¿cuál es el cuello de botella más probable?
...
377. ¿Qué representa la métrica `nodejs_heap_size_used_bytes` en la monitorización de una API Node.js con Prometheus?
...
378. ¿Es necesario tener el servicio SSH para que Ansible funcione?
...
379. En Prometheus, ¿qué es un `job_name` dentro de `scrape_configs` en el archivo `prometheus.yml`?
...
380. ¿Cuál es el propósito de --ask-become-pass en el comando ansible-playbook -i hosts.ini playbook.yml --ask-pass --ask-become-pass?
...
381. Al configurar un servidor web como Apache o Nginx y habilitar su acceso por firewall, ¿qué puerto es el estándar para HTTP?
...
382. ¿Para qué se utiliza principalmente la herramienta nmap en la administración de servidores y redes?
...
383. ¿Cuál es una de las funciones de `docker compose up -d`?
...
384. ¿Para qué sirve la herramienta Apache Benchmark (`ab`)?
...
385. ¿Para qué se utiliza `cron` en un sistema Linux?
...
386. ¿Qué archivos de configuración en Zabbix hay que modificar en las prácticas?
...
387. En un playbook de Ansible, ¿qué efecto tiene la directiva become: true a nivel de play o tarea?
...
388. En LVM, si se necesita expandir un Logical Volume (LV) y hay espacio libre en el Volume Group (VG), ¿qué comando se podría usar?
...
389. En el archivo docker-compose.yml se especifica la imagen prom/prometheus:v2.50.0. Si se descubre una vulnerabilidad crítica en la versión v2.50.0 de Prometheus, ¿cuál sería la acción más recomendable y segura a seguir, asumiendo que existe una versión parcheada como v2.50.1?
...
390. ¿Cuál de los siguientes NO es un componente de LVM según el diagrama de las transparencias?
...
391. la opción -X de ssh sirve para:
...
392. Phoronix Test Suite (PTS) se integra con OpenBenchmarking para comparar resultados. ¿Qué ventaja estratégica principal ofrece esta integración a un usuario que evalúa el rendimiento de un nuevo hardware o configuración?
...
393. En Ansible, ¿cómo interactúan notify y handlers?
...
395. ¿Qué ventaja nos da JWT sobre otras autenticaciones?
...
396. ¿Qué empresa es responsable de VirtualBox?
...
397. Un script cron usa `logger -t MYSCRIPT "Mensaje"`. El sistema usa `rsyslog`. Sin configuración específica para "MYSCRIPT", ¿dónde irán los mensajes y cuál es el problema si `/var/log/syslog` está muy concurrido?
...
398. ¿Cuál es la distinción clave entre "monitorización" y "alerting" en el contexto de sistemas como Prometheus y Grafana?
...
399. ¿Para qué se utiliza el módulo authorized_key de Ansible en la tarea "Añadir clave pública SSH para admin"?
...
400. En la estructura de JMeter, bajo "Login Administradores", ¿qué parámetros se envían en la petición HTTP POST?
...
401. Si un objetivo de scraping (target) configurado en prometheus.yml, como una instancia de node_exporter en host-servidor:9100, está inaccesible (por ejemplo, el host está apagado o el servicio node_exporter no se está ejecutando), ¿qué estado mostrará Prometheus para este target en su interfaz web (sección Targets)?
...
402. ¿Qué es un "hipervisor" en el contexto de la virtualización?
...
403. ¿Qué herramienta de gestión de paquetes está disponible en Centos?
...
404. En el comando mdadm --create --verbose /dev/md0 ..., ¿cuál es el propósito de la opción --verbose?
...
405. ¿Cuál de los siguientes es un ejemplo de Cloud Pública según las transparencias?
...
406. ¿Qué comando emplearía en Linux para revisar los mensajes que ha generado el SO durante su arranque?
...
407. En "HTTP Request Defaults" ("Acceso API"), se especifican `${HOST}` y `${PORT}`. Si un "HTTP Request" individual dentro del mismo ámbito omite el "Server Name or IP" pero especifica un "Path" diferente al de "HTTP Request Defaults" (que está vacío), ¿cómo se construye la URL final de la petición?
...
408. ¿Qué es la virtualización de servidores según el contexto de las prácticas?
...
409. Para la instalación del SO Rocky Linux en las prácticas, ¿se recomienda instalar un entorno gráfico?
...
410. ¿A qué puerto haces las peticiones JMeter en la aplicación de prácticas?
...
411. En Grafana, al configurar Prometheus como "Datasource", ¿qué URL se utiliza si Grafana y Prometheus corren como servicios en el mismo `docker-compose.yml`?
...
412. Si en el archivo `prometheus.yml` se define un `target` como `mi_servidor_rocky:9100`, ¿a qué puerto se conectará Prometheus en `mi_servidor_rocky` para obtener métricas del Node Exporter?
...
413. Si el directorio `/boot` de un sistema Linux se llena, ¿cuál podría ser una consecuencia directa?
...
414. La función `rate(http_requests_total[5m])` en PromQL calcula:
...
415. ¿Cual de las siguientes afirmaciones es correcta?
...
417. ¿Qué es un "Assertion" en JMeter?
...
418. Un script de shell contiene la línea: ansible-playbook -i inventory/hosts.ini playbooks/configurar_web.yml. ¿Qué acción principal realiza este script?
...
419. ¿Qué comando se utiliza comúnmente en Linux para verificar el estado actual de los arreglos RAID gestionados por software (mdadm)?
...
421. Las siglas LAMP hacen referencia a:
...
422. ¿Qué tipo de información se encuentra típicamente en `/proc/meminfo` en un sistema Linux?
...
423. Si el "Gaussian Random Timer" (Desviación 100ms, Offset 300ms) se coloca como hijo directo de una petición HTTP Sampler en lugar de a nivel de Thread Group o Controlador, ¿cuándo se ejecutará la pausa introducida por este temporizador?
...
424. En VirtualBox, ¿cuál es la diferencia fundamental entre un "Snapshot" y "Clonar" una máquina virtual?
...
425. Si en Grafana se observa que un panel que muestra métricas de Prometheus no actualiza datos o muestra "N/A", una posible causa NO sería:
...
426. Para investigar problemas con un servicio específico (por ejemplo, nginx.service) durante el arranque actual, utilizando journalctl, ¿qué comando filtraría los mensajes de log para esta unidad específica, mostrando solo aquellos con una prioridad de 'error' o más grave?
...
427. Phoromatic es una interfaz que se utiliza para
...
428. La arquitectura de Microservicios implica:
...
429. ¿Qué software de virtualización se indica explícitamente para homogeneizar el entorno de prácticas en ISE?
...
430. En RAID 5 (Paridad distribuida), si un disco falla, ¿cómo se recuperan los datos?
...
432. ¿Qué contiene el archivo que se utiliza en el Access Log Sampler?
...
433. ¿Qué permite hacer la opción "Export for sharing externally" al compartir un dashboard en Grafana?
...
434. ¿Qué son los "módulos" en Ansible?
...
435. En el archivo docker-compose.yml, el servicio de Prometheus tiene una sección command con el argumento --config.file=/etc/prometheus/prometheus.yml. ¿Cuál es la implicación principal de este comando para el contenedor de Prometheus?
...
436. El comando Apache Benchmark (ab) ab -n 100 -c 10 http://localhost/ realiza 100 peticiones con una concurrencia de 10. Si se incrementara significativamente solo el valor de -c (concurrencia) manteniendo -n constante, ¿qué aspecto del rendimiento del servidor se estaría poniendo a prueba de forma más intensiva?
...
437. ¿Qué le ocurre a Ubuntu Server cuando quitamos uno de los dos discos virtuales?
...
438. ¿Qué cabecera del protocolo http devuelve el servidor web para informar que el contenido está comprimido?
...
439. En el Dockerfile de una aplicación Node.js, se usa `ENV NODE_ENV=production`. ¿Cuál es una consecuencia común de esto para frameworks como Express?
...
440. En la sección de handlers, el handler "Reiniciar Apache" tiene una cláusula when: web_package == "httpd". ¿Cuál es el propósito de esta condición dentro del handler?
...
442. En JMeter, ¿qué función cumple un "HTTP Cookie Manager"?
...
443. Al configurar permisos para el volumen ./grafana_data, la documentación especifica sudo chmod -R 755 ./grafana_data. ¿Cuál sería el principal riesgo de seguridad si, en lugar de 755, se utilizara despreocupadamente chmod -R 777 ./grafana_data en un sistema multiusuario?
...
444. El archivo prometheus.yml define una configuración global y un trabajo de scraping. Si el scrape_interval global es de 5s y el único job_name es "prometheus_service" que apunta a "prometheus:9090", ¿qué significa esto para la recolección de métricas del propio Prometheus?
...
446. Si se desea volver al modo de operación normal multiusuario desde el modo de rescate en un sistema systemd, ¿qué comando se utilizaría?
...
447. ¿Cómo se busca un paquete en apt?
...
448. ¿Cuál es la diferencia crítica entre `EXPOSE 3000` en Dockerfile y `-p 8080:3000` en `docker run` para la accesibilidad de red del contenedor?
...
449. `logrotate` se ejecuta diariamente vía cron. Config en `/etc/logrotate.d/myservice`: `size 10M`, `rotate 4`, `daily`. Si el log de `myservice` crece a 50MB en 12 horas. ¿Cuándo y cómo se rotarán?
...
450. Para importar un dashboard predefinido en Grafana desde Grafana.com/dashboards, ¿qué se suele necesitar?
...
451. ¿Cuál es el propósito general del módulo lineinfile de Ansible, como se usa en la tarea "Permitir acceso root por contraseña temporalmente"?
...
452. ¿Qué es Ansible, según se describe en el material?
...
454. ¿Qué formato de archivo se utiliza comúnmente para escribir playbooks de Ansible?
...
455. ¿Qué información proporciona el comando `lsblk`?
...
456. En un `prompt` de bash como `[admin@dpsMV01-17:30:25 etc]$`, ¿qué representa `dpsMV01`?
...
457. ¿Podríamos comparar el performance de dos servidores realizando test desde una máquina a la otra y viceversa?
...
458. En el contexto de la seguridad SSH, ¿qué es un "man-in-the-middle attack"?
...
459. Al hacer los tests obtenemos el valor de respuesta 401 porque...
...
460. ¿Qué comando se utiliza para verificar la conectividad de red con otro host enviando paquetes ICMP Echo Request?
...
461. ¿Qué beneficio puede tener usar LVM?
...
462. Al configurar SSH para acceso remoto sin contraseña usando claves, ¿dónde se almacena la clave pública del cliente en el servidor?
...
464. La documentación indica que para los volúmenes de datos de Docker, como ./prometheus_data, se ejecuta sudo chown -R 65534:65534 ./prometheus_data. ¿Cuál es la razón técnica más probable para utilizar el UID/GID 65534:65534 en este contexto específico?
...
465. El comando `scp` (Secure Copy) se utiliza para:
...
466. ¿Para qué usa en la práctica el elemento Regular Expression Extractor de JMeter?
...
467. ¿Por qué es necesario tener la pila LAMP instalada para usar Zabbix server?
...
468. En el archivo `docker-compose.yml` de ejemplo para Prometheus y Grafana, ¿qué puerto del host se mapea al puerto 3000 del contenedor Grafana?
