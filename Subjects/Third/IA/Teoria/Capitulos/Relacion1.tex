...
% La ruta que había aquí era la del sitio antiguo y ya no resuelve. El código
% tampoco se publica: el marco de la práctica es del profesorado y solo el
% comportamiento del agente es propio.
\textbf{El código de los ejercicios 1, 3 y 4 está en el repositorio, bajo
\texttt{Subjects/Third/IA/Teoria/Capitulos/Elementos\_Relacion\_1}.}
...
Hay que tener en cuenta las características de los agentes reactivos:
\begin{itemize}
    \item Sensores:
    \begin{itemize}
...
Con esto el agente queda definido por completo.

\subsubsection*{Anotaciones sobre el desarrollo del ejercicio}
\begin{itemize}
    \item La solución falla porque siempre gira hacia la derecha, así que hay que recurrir a las variables de estado para solucionar este problema.
    \item Queda el problema de que, si se cruzan los caminos, no pasará por uno de ellos.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Si estoy\_contando y no estoy\_girando y CNY70 $\rightarrow$ Termina, y se devuelve el \underline{IDLE} y el \underline{contador}.
        \end{itemize}
\end{itemize}
...
Sea un robot sobre un mapa bidimensional discreto de tamaño NxM. El 
...
Como solución, al elegir uno en función del potencial se toma D.
...
La casilla del cuadrado es la que menos potencial tiene por lo que se toma esa (Ecuación 1.1).
