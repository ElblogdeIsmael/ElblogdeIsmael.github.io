# Test de Contabilidad Financiera-T1

* **Autor:** Ismael Sallami Moreno
* **Titulación:** Doble Grado en Ingeniería Informática y ADE

1. ¿Qué organismo en España regula la contabilidad y auditoría?

    - (x) ICAC
    - ( ) AECA
    - ( ) FASB
    - ( ) IASB

2. ¿Qué documento incluye información sobre el origen y uso de los activos monetarios?

    - ( ) Balance
    - (x) Estado de Flujos de Efectivo
    - ( ) Memoria
    - ( ) Cuenta de Pérdidas y Ganancias

3. ¿Cuál es el principal objetivo de la normalización contable?

    - ( ) Reducir la cantidad de información contable
    - (x) Proporcionar criterios uniformes y comparables en la información financiera
    - ( ) Aumentar las divergencias contables
    - ( ) Facilitar la evasión fiscal

4. ¿Cuáles son los requisitos para presentar un Balance abreviado?

    - ( ) Total activo inferior a 11.400.000 €
    - ( ) Importe neto de la cifra de negocios inferior a 22.800.000 €
    - (x) Número medio de trabajadores inferior a 50
    - ( ) Ninguna de las anteriores

5. ¿Qué parte del PGC se ocupa de los principios contables?

    - ( ) Segunda parte
    - (x) Primera parte
    - ( ) Tercera parte
    - ( ) Ninguna de las anteriores

6. ¿Qué significa “imagen fiel” en la contabilidad?

    - ( ) Reflejar la realidad jurídica de la empresa
    - (x) Reflejar la realidad económica y financiera de la empresa
    - ( ) Reflejar las expectativas del mercado
    - ( ) Reflejar solo los activos

7. ¿Qué característica cualitativa asegura que la información contable esté libre de errores?

    - ( ) Claridad
    - ( ) Comparabilidad
    - (x) Fiabilidad
    - ( ) Relevancia

8. ¿Qué organismo emite las Normas Internacionales de Información Financiera (NIIF)?

    - ( ) FASB
    - (x) IASB
    - ( ) ICAC
    - ( ) AECA

9. ¿Qué documento refleja el resultado del ejercicio económico?

    - ( ) Balance
    - ( ) Memoria
    - (x) Cuenta de Pérdidas y Ganancias
    - ( ) Estado de Cambios en el Patrimonio Neto

10. ¿Qué documento incluye información adicional para interpretar los estados financieros?

    - (x) Memoria
    - ( ) Balance
    - ( ) Estado de Flujos de Efectivo
    - ( ) Cuenta de Pérdidas y Ganancias

11. ¿Cuál de los siguientes documentos es opcional en el modelo abreviado?

    - ( ) Balance
    - ( ) Memoria
    - (x) Estado de Cambios en el Patrimonio Neto
    - ( ) Cuenta de Pérdidas y Ganancias

12. ¿Cuál es el requisito de activos para presentar una Cuenta de Pérdidas y Ganancias abreviada?

    - (x) Activos inferiores a 11.400.000 €
    - ( ) Activos inferiores a 8.000.000 €
    - ( ) Activos inferiores a 4.000.000 €
    - ( ) Ninguno de los anteriores

13. ¿Qué significa el concepto de "normas de registro y valoración" en el PGC?

    - ( ) Criterios para presentar el balance
    - (x) Directrices para el tratamiento contable de elementos como activos, pasivos e ingresos
    - ( ) Métodos de análisis financiero
    - ( ) Ninguna de las anteriores

14. ¿Qué organismo tiene sede en Londres y emite las NIIF?

    - ( ) ICAC
    - ( ) AECA
    - (x) IASB
    - ( ) FASB

15. ¿Qué significa el término "fiabilidad" en la información contable?

    - (x) Que la información es veraz y libre de sesgos
    - ( ) Que la información siempre debe ser perfecta
    - ( ) Que la información es compleja
    - ( ) Que la información es irrelevante

16. ¿Qué parte del PGC define los elementos como activos y pasivos?

    - ( ) Normas de Registro y Valoración
    - (x) Marco Conceptual
    - ( ) Cuentas Anuales
    - ( ) Ninguna de las anteriores

17. ¿Qué tipo de norma utiliza el ICAC para resolver cuestiones contables concretas?

    - (x) Resoluciones
    - ( ) Consultas
    - ( ) Adaptaciones sectoriales
    - ( ) Ninguna de las anteriores

18. ¿Cuál de las siguientes es una ventaja de la normalización contable internacional?

    - (x) Favorece la transparencia en los informes financieros
    - ( ) Aumenta los costes contables
    - ( ) Impide la comparación entre empresas
    - ( ) Reduce la competitividad

19. ¿Qué organismo en Europa evalúa la adopción de las NIIF emitidas por el IASB?

    - (x) EFRAG
    - ( ) FASB
    - ( ) AECA
    - ( ) ICAC

20. ¿Cuál es el principal objetivo de las NIIF?

    - ( ) Facilitar la evasión fiscal
    - (x) Mejorar la transparencia y comparabilidad en los informes financieros
    - ( ) Generar más información contable
    - ( ) Aumentar la complejidad de los estados financieros

21. ¿Cuáles son las características que deben tener los estados contables para que los usuarios puedan tomar decisiones con mayor relevancia y fiabilidad?

    - (x) Relevante, comparable y fiable.
    - ( ) Relevante, homogéneo y veraz.
    - ( ) Comparable, homogéneo y completo.
    - ( ) Fiable, veraz y homogéneo.

22. ¿Cuál de los siguientes documentos no forma parte de las cuentas anuales según el Marco Conceptual?

    - ( ) El Balance.
    - ( ) La Cuenta de Pérdidas y Ganancias.
    - ( ) El Estado de Cambios en el Patrimonio Neto.
    - (x) El Estado de Ingresos y Gastos.

23. ¿Cuál es el objetivo de presentar los documentos de las cuentas anuales de manera conjunta?

    - ( ) Reducir el volumen de información.
    - (x) Ofrecer una visión completa de la empresa.
    - ( ) Cumplir con requisitos legales.
    - ( ) Facilitar la auditoría interna.

24. ¿Qué característica cualitativa de la información financiera permite contrastar la información entre diferentes empresas y periodos?

    - ( ) Relevancia.
    - ( ) Fiabilidad.
    - (x) Comparabilidad.
    - ( ) Claridad.

25. Según el Marco Conceptual, ¿qué implica que la información financiera sea fiable?

    - (x) Que esté libre de errores materiales y sesgos.
    - ( ) Que sea exacta en todos los detalles.
    - ( ) Que sea fácil de entender.
    - ( ) Que se haya auditado correctamente.

26. ¿Qué elemento de los estados financieros se define como "bienes, derechos y otros recursos controlados por la empresa, resultantes de sucesos pasados, de los que se espera obtener beneficios o rendimientos económicos en el futuro"?

    - (x) Activos.
    - ( ) Pasivos.
    - ( ) Patrimonio Neto.
    - ( ) Ingresos.

27. ¿Qué elemento de los estados financieros representa la "parte residual de los activos, deducidos todos sus pasivos"?

    - ( ) Activos.
    - ( ) Pasivos.
    - (x) Patrimonio Neto.
    - ( ) Ingresos.

28. Según el ejemplo dado en el contexto, ¿qué acción debe realizar una empresa con un inmovilizado material que no cumple la definición de activo?

    - ( ) Amortizarlo anualmente.
    - ( ) Mantenerlo en el balance como activo no corriente.
    - (x) Darlo de baja del balance.
    - ( ) Venderlo inmediatamente.

29. La empresa "Zeta" ha sido demandada por un empleado despedido, y se estima un pago de indemnización de 160.000 €. ¿Qué debe hacer la empresa al reconocer este pasivo?

    - (x) Reconocer la provisión por el coste estimado de 160.000 €.
    - ( ) Esperar a que se confirme el importe exacto para reconocer el pasivo.
    - ( ) No reconocer nada hasta que finalice el proceso judicial.
    - ( ) Registrar un gasto por el importe de 160.000 €.

30. Según el PGC, ¿cómo se define el patrimonio neto?

    - ( ) La suma total de activos y pasivos.
    - ( ) Las aportaciones de los socios y propietarios.
    - (x) La parte residual de los activos, una vez deducidos todos los pasivos.
    - ( ) Los resultados acumulados de la empresa.

31. ¿Cuál de los siguientes no es un componente del patrimonio neto según el PGC?

    - ( ) Fondos propios.
    - ( ) Ajustes por cambios de valor.
    - ( ) Subvenciones, donaciones y legados recibidos.
    - (x) Pasivos a largo plazo.

32. ¿Qué deben cumplir los ingresos para ser reconocidos en la Cuenta de Pérdidas y Ganancias según el PCC?

    - ( ) Incrementos en el patrimonio neto durante el ejercicio.
    - ( ) Proceder de aportaciones de los socios o propietarios.
    - ( ) Representar entradas o aumentos en el valor de los activos.
    - (x) Ser incrementos en el patrimonio neto que no tengan su origen en aportaciones de los socios.

33. ¿Cuál de los siguientes ejemplos de gastos se incluye en la Cuenta de Pérdidas y Ganancias?

    - ( ) Venta de mercaderías.
    - ( ) Ingresos por arrendamientos.
    - (x) Primas de seguros.
    - ( ) Ingresos por prestación de servicios.

34. Según el PGC, ¿cuál de los siguientes documentos no se considera parte de los principios y normas de contabilidad generalmente aceptados?

    - ( ) El Código de Comercio.
    - ( ) Las normas de desarrollo del Instituto de Contabilidad y Auditoría de Cuentas.
    - ( ) La legislación española aplicable.
    - (x) Las Normas Internacionales de Información Financiera.

35. ¿Qué principio establece que la gestión de la empresa continuará en un futuro previsible?

    - ( ) Principio de Devengo.
    - (x) Principio de Empresa en Funcionamiento.
    - ( ) Principio de Uniformidad.
    - ( ) Principio de No Compensación.

36. ¿Qué principio fundamenta la periodificación de los gastos e ingresos?

    - ( ) Principio de Empresa en Funcionamiento.
    - (x) Principio de Devengo.
    - ( ) Principio de Prudencia.
    - ( ) Principio de Importancia Relativa.

37. Según el PCGC, ¿qué debe hacerse al cambiar un criterio contable por alteración de supuestos?

    - ( ) Aplicar el nuevo criterio en el siguiente ejercicio.
    - ( ) Mantener el criterio anterior.
    - (x) Mencionar el cambio en la Memoria, indicando su incidencia cuantitativa y cualitativa.
    - ( ) No realizar ningún cambio.

38. ¿Cuál es el propósito del principio de Prudencia?

    - ( ) Asegurar que los beneficios se contabilicen antes de su realización.
    - ( ) Garantizar que los riesgos y beneficios se reconozcan por igual.
    - (x) Ser prudente en estimaciones y valoraciones bajo incertidumbre.
    - ( ) Permitir la compensación de gastos e ingresos.

39. ¿Qué principio establece que no deben compensarse las partidas del activo y del pasivo, ni las de gastos e ingresos?

    - (x) Principio de No Compensación.
    - ( ) Principio de Importancia Relativa.
    - ( ) Principio de Devengo.
    - ( ) Principio de Empresa en Funcionamiento.

40. ¿Cuándo se admite la no aplicación estricta de algunos principios y criterios contables?

    - ( ) Cuando la importancia relativa de la variación sea significativa.
    - (x) Cuando no altere la expresión de la imagen fiel.
    - ( ) Cuando se considere necesario por la administración.
    - ( ) Cuando afecte al balance general.

41. ¿Qué incluye el coste de producción de un activo?

    - ( ) El precio de adquisición de las materias primas y otras materias consumibles.
    - ( ) El importe en efectivo y otras partidas equivalentes pagadas o pendientes de pago.
    - ( ) Los factores de producción directamente imputables al activo.
    - (x) El precio de adquisición de las materias primas y otras materias consumibles, los factores de producción directamente imputables al activo, y la fracción correspondiente de los costes de producción indirectamente relacionados.

42. ¿Qué se incluye en la estimación del valor razonable de un activo no financiero?

    - ( ) El estado de conservación y la ubicación del activo.
    - (x) El estado de conservación y la ubicación del activo, y las restricciones sobre la venta o el uso del activo.
    - ( ) Las restricciones sobre la venta o el uso del activo.
    - ( ) La capacidad de uso del activo.

43. ¿Cuál es la mejor referencia del valor razonable según el PGC?

    - ( ) El precio de mercado en un mercado no activo.
    - (x) El precio cotizado en un mercado activo.
    - ( ) El valor estimado por la empresa.
    - ( ) El precio histórico.

44. ¿Qué niveles de jerarquía se utilizan para la estimación del valor razonable?

    - ( ) Nivel 1, Nivel 2, Nivel 4.
    - (x) Nivel 1, Nivel 2, Nivel 3.
    - ( ) Nivel 2, Nivel 3, Nivel 4.
    - ( ) Nivel 3, Nivel 4, Nivel 5.

45. ¿Qué se asume para la transacción al calcular el valor razonable?

    - ( ) Entre partes no interesadas y mal informadas.
    - ( ) En el mercado menos ventajoso.
    - (x) Entre partes interesadas y debidamente informadas, en una transacción en condiciones de independencia mutua.
    - ( ) Entre partes dependientes.

46. ¿Qué debe considerarse al ajustar el tipo de interés de mercado para calcular el valor en uso?

    - ( ) Solo los riesgos generales de mercado.
    - ( ) Solo los riesgos específicos del activo.
    - ( ) Los riesgos generales de mercado y los riesgos específicos del activo.
    - (x) Los riesgos específicos del activo, apoyándose en hipótesis sobre los riesgos considerados por los participantes en el mercado.

47. ¿Qué excluyen los costes de ventas según el PGC?

    - ( ) Los gastos legales necesarios para transferir la propiedad del activo.
    - ( ) Las comisiones de venta.
    - ( ) Los impuestos sobre beneficios y los gastos financieros.
    - (x) Los impuestos sobre beneficios y los gastos financieros, y los costes en que la empresa no habría incurrido si no hubiera decidido vender el activo.

48. ¿Qué se incluye entre los costes de transacción atribuibles a un activo o pasivo financiero?

    - ( ) Las primas obtenidas en la compra.
    - ( ) Los gastos financieros.
    - ( ) Los costes de mantenimiento.
    - (x) Los honorarios y comisiones pagadas a agentes, asesores e intermediarios, los gastos de intervención de fedatario público y los impuestos.

49. ¿Qué método de cálculo del valor razonable se debe usar preferentemente según el PGC?

    - ( ) Modelos y técnicas de valoración.
    - ( ) Coste amortizado.
    - ( ) Coste histórico.
    - (x) Valor fiable de mercado.

50. ¿Qué debe utilizarse para calcular el cuadro de amortización a coste amortizado de un préstamo?

    - ( ) El tipo de interés nominal.
    - ( ) La comisión de apertura.
    - ( ) El tipo de interés efectivo.
    - (x) El tipo de interés efectivo, teniendo en cuenta las comisiones de estudio y apertura.
