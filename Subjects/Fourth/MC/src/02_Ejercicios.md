...
<!-- \begin{solucion}
    ...
    El lenguaje generado por la gramática \text{sí es regular}.

    La justificación se basa en los siguientes puntos clave:

    \begin{enumerate}
        \item \text{Análisis del lenguaje de \( X \):} La variable \( X \) genera todas las palabras que tienen \text{exactamente una b} y cualquier cantidad de 'a's (\( a^*ba^* \)). Este es un lenguaje regular.

        \item \text{Análisis del lenguaje de \( S \):}
        \begin{enumerate}
            \item La regla \( S \to XXX \) concatena tres palabras de \( X \). Como cada una tiene una b, el resultado son palabras con \text{exactamente tres b}.
            \item La regla \( S \to SS \) permite concatenar estas palabras entre sí, lo que resulta en palabras cuyo número de b es siempre un \text{múltiplo positivo de 3} (3, 6, 9, etc.).
        \end{enumerate}

        \item \text{Conclusión sobre la regularidad:} El lenguaje \( L(G) \) es regular porque:
        \begin{enumerate}
            ...
            \item Se puede diseñar un \text{autómata finito} con tres estados que lo reconozca, contando el número de b módulo 3. Esto se verá más en detalle a lo largo del curso.
        \end{enumerate}
    \end{enumerate}
