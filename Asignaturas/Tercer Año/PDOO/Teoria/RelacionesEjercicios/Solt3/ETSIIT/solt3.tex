...
public class Articulo extends Documento {
    ...
    public Articulo(String titulo, Date fecha, int pagIni, int pagFin);
...
\begin{itemize}
    \item No se puede instanciar directamente.
    \item Contiene al menos un método abstracto.
\end{itemize}

En este modelo, ¿puede haber artículos que no estén ligados a una revista?
\begin{itemize}
    \item No, todos los artículos deben estar ligados a una revista.
\end{itemize}
...
\begin{itemize}
    \item Documento - Articulo: H
    \item Documento - Libro: H
    \item Documento - Revista: H
    \item BibliotecaElectronica - Documento: AG
    \item Articulo - Revista: C
    \item Documento - Imprimible: R
\end{itemize}
...
\begin{itemize}
    \item El método \texttt{imprimir()} está redefinido porque sobrescribe el método de la clase \texttt{Documento}.
\end{itemize}

Corrige el código:
\begin{lstlisting}[style=customjava]
Imprimible docu = new Articulo("Intemperie", fecha, 1, 10);
docu.imprimir();
\end{lstlisting}
...
\begin{itemize}
    \item Hay un error de compilación en \texttt{docu.imprimir(pagIni, pagFin);} porque el método \texttt{imprimir()} no acepta parámetros en la clase \texttt{Documento}.
\end{itemize}
