...
\begin{tikzpicture}[node distance=0.8cm, auto, scale=0.75, transform shape]
    ...
    %Main class
    \node[class] (Vehiculo) {\textbf{Vehiculo} \\ -marca : String \\ -capacidad : double \\ -estado : String \\ +desplazarse() \\ +acelerar()};    
    
    % Interface
    \node[class, right=1cm of Vehiculo] (Transporte) {\textit{<<interface>>} \\ \textbf{Transporte} \\ +desplazarse()};
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{6cm}|p{6cm}|}
        \hline
        \textbf{Código }& \textbf{Error si procede y corrección}\\
        \hline
        Transporte x = new Barco();
        x.atracar(); & Error de compilación, ya que el tipo de la variable x es Transporte y no tiene el método atracar.\\
        \hline
        Avion av = new Avion();
av.acelerar(); & Correcto.\\
        \hline
        Vehiculo v = new Vehiculo();
v.desplazarse(); & Correcto.\\
        \hline
        Vehiculo v2 = new Vehiculo();
v2.acelerar(); & Correcto.\\
        \hline
        Transporte t = new Avion():
Barco b= new Barco();
t = b; & Error de compilación, ya que no se puede asignar un objeto de tipo Barco a una variable de tipo Transporte.\\
        \hline
        Avion a = new Avion();
String est = a.estado; & Error de compilación, ya que el atributo estado es privado en la clase Vehiculo.\\
        \hline
        Vehiculo v3 = new Barco();
((Barco) v3).atracar(); & Correcto.\\
        \hline
        List<Transporte> listaTransportes = new
ArrayList();
...
tr.acelerar();
tr.atracar();}  & Error de compilación, ya que el método atracar() no está definido en la interfaz Transporte.\\
        \hline
...
((Barco) tr).acelerar();
((Barco) tr).atracar();} & Error de ejecución, ya que no se puede hacer un cast de un objeto de tipo Avion a Barco.\\
        \hline
    \end{tabular}
\end{table}
...
En este modelo si, lo que no puede haber son revistas que no estén ligadas a un Artículo debido a que según el diagrama de clases y la lógica el rombo de la flecha de composición parte de la clase Artículo.
...
\begin{lstlisting}[style=customjava, caption={VentanaBiblioteca}]
...
\begin{itemize}
    \item El método \texttt{imprimir()} está redefinido porque sobrescribe el método de la clase \texttt{Documento}, el cual lo implementa de la interfaz \texttt{Imprimible}.
    \item El método \texttt{imprimir(int numeroPagina)} está sobrecargado porque tiene la misma firma que el método \texttt{imprimir()} pero con un parámetro adicional, el cual lo implementa de la interfaz \texttt{Imprimible}.
    \item El método \texttt{imprimir(int pagInicio, int pagFin)} está sobrecargado porque tiene la misma firma que el método \texttt{imprimir()} pero con dos parámetros adicionales, el cual lo implementa de la interfaz \texttt{Imprimible}.
    \item El método \texttt{numeroPaginas()} está redefinido porque sobrescribe el método de la clase \texttt{Documento}.
\end{itemize}
...
\begin{enumerate}
    \item Sirve como clase plantilla para las clases Libro y Revista que son clases hijas según el diagrama de clases.
    \item Contiene al menos un método abstracto, que en este caso es el método \texttt{numero\_paginas}.
\end{enumerate}

\subsubsection*{¿Puede haber artículos que no estén ligados a una revista?}

En este modelo sí, lo que no puede haber son revistas que no estén ligadas a un Artículo debido a que según el diagrama de clases y la lógica el rombo de la flecha de composición parte de la clase Artículo.
...
\begin{lstlisting}[style=customrb, caption={VentanaBiblioteca}]
    class VentanaBiblioteca
...
\begin{itemize}
    \item El método \texttt{imprimir} está redefinido porque sobrescribe el método de la clase \texttt{Documento}, el cual lo implementa de la interfaz \texttt{Imprimible}.
    \item El método \texttt{imprimir\_por\_pagina} está sobrecargado porque tiene la misma firma que el método \texttt{imprimir}, pero con un parámetro adicional, el cual lo implementa de la interfaz \texttt{Imprimible}.
    \item El método \texttt{imprimir\_por\_rango} está sobrecargado porque tiene la misma firma que el método \texttt{imprimir}, pero con dos parámetros adicionales, el cual lo implementa de la interfaz \texttt{Imprimible}.
    \item El método \texttt{numero\_paginas} está redefinido porque sobrescribe el método de la clase \texttt{Documento}.
\end{itemize}

\subsubsection*{Corrige el código}

    \begin{lstlisting}[style=customrb, caption = {Código a corregir}]
    imp = Documento.new("Intemperie", fecha)  # suponiendo que fecha está inicializada
    imp.imprimir
    \end{lstlisting}

Corrección:
\begin{lstlisting}[style=customrb, caption={Corrección del código}]
    imp = Articulo.new("Intemperie", fecha, 1, 10)
    imp.imprimir
\end{lstlisting}

\subsubsection*{Corrige el código:}
    \begin{lstlisting}[style=customrb, caption = {Código a corregir}]
    docu = Libro.new("ISBN10102030")
    codigo = docu.isbn
    \end{lstlisting}
...
\subsubsection*{Rellena la siguiente tabla indicando el tipo estático y dinámico de la variable docu en las siguientes líneas de código:}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{|p{6cm}|l|l|}
            \hline
            \textbf{Código} & \textbf{Tipo estático} & \textbf{Tipo dinámico} \\ \hline
            docu = Articulo.new(titulo, fecha, pag\_ini, pag\_fin) & Documento & Articulo \\ \hline
            docu.imprimir(pag\_ini, pag\_fin) & Documento & Articulo \\ \hline
            docu = Libro.new(titulo, fecha, isbn, pags) & Documento & Libro \\ \hline
            docu.imprimir & Documento & Libro \\ \hline
        \end{tabular}
    \end{table}

\subsubsection*{Indica si hay errores de compilación o ejecución en el código anterior (suponiendo que las variables titulo, fecha, pag\_ini, pag\_fin, isbn y pags han sido declaradas e inicializadas convenientemente con anterioridad). Justifica tu respuesta.}

\begin{itemize}
    \item En la primera línea de código no hay errores de compilación ni de ejecución.
    \item En la segunda línea no hay errores de compilación ni de ejecución.
    \item En la tercera línea no hay errores de compilación ni de ejecución.
    \item En la cuarta línea no hay errores de compilación ni de ejecución.
\end{itemize}

\subsubsection*{Rellena la siguiente tabla marcando con una “X” la situación que corresponda a cada una de las líneas del siguiente bloque de código (suponiendo que las variables titulo, fecha, pag\_ini y pag\_fin han sido declaradas e inicializadas convenientemente con anterioridad).}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{4cm}|p{4cm}|p{4cm}|p{4cm}|}
        \hline
        \textbf{Código} & \textbf{Ningún error} & \textbf{Sólo error de compilación} & \textbf{Sólo error de ejecución} \\ \hline
        imp = Articulo.new(titulo, fecha, pag\_ini, pag\_fin) & X &  &  \\ \hline
        docu = imp &  & X &  \\ \hline
        imp.numero\_paginas &  & X &  \\ \hline
        docu.isbn &  &  & X \\ \hline
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    correr() & No, porque es una redefinición   \\ \hline
    comerSano() &  \\ \hline
    entrenar() &  \\ \hline
    entrenar(nombreEntrenador:String) &  \\ \hline
    \end{tabular}
