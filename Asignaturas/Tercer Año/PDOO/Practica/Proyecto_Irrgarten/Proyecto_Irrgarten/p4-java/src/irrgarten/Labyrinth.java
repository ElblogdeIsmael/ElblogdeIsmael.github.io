...
public class Labyrinth {
    ...
    /**
     * Añade el número en formato char que se le pasa como argumento al laberinto
     * @param number número que se añade al laberinto
     * @param row fila en la que se añade el número
     * @param col columna en la que se añade el número
     */
    public void addNumberLabyrinth(int row, int col, char number) {
        labyrinth[row][col] = number;
        //SE PUEDE USAR, PERO NO LO HACEMOS, SE PUEDE ELIMINAR, LINEA 407 PUTPLAYER2D
    }
    /**
    ...
    public void spreadPlayers(Player players[]) {
        for(int i=0;i<players.length;i++){
            ...
            int oldRow=-1; // es -1 porque no tenemos posiciones anteriores
            int oldCol=-1;
    ...
    public void updateOldPos(int row, int col) {
        if (posOK(row, col)) {
            ...
            System.out.println("Actualizando posición anterior....\n");
    ...
    public Directions[] validMoves(int row, int col) {
        ...
        return validMoves.toArray(new Directions[0]);
